...
define float @fadd_float_const(float %a) {
; CHECK-LABEL: (func $fadd_float_const {{.*}} (param f32) (result f32)
...
define double @fadd_double_const(double %a) {
; CHECK-LABEL: (func $fadd_double_const {{.*}} (param f64) (result f64)
; CHECK-NEXT:    local.get 0
; CHECK-NEXT:    f64.const {{.*}} (;=133.7;)
; CHECK-NEXT:    f64.add
entry:
  %0 = fadd double %a, 133.7
  ret double %0
}
