...
Opcode::Info Opcode::getInfo() const {
  ...
  Info InvalidInfo = Infos[Opcode::Invalid];
...
bool Opcode::isEnabled(const Features &Fs) const {
  ...
  case Opcode::Rethrow:
    return Fs.exceptions_enabled();

  case Opcode::ReturnCallIndirect:
  case Opcode::ReturnCall:
    return Fs.tail_call_enabled();
  ...
  case Opcode::I32X4DotI8X16I7X16AddS:
    return Fs.relaxed_simd_enabled();
  ...
  case Opcode::RefIsNull:
    return Fs.reference_types_enabled();

  case Opcode::CallRef:
    return Fs.function_references_enabled();
