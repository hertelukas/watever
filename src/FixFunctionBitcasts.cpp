...
static Function *createWrapper(Function *F, FunctionType *Ty) {
  ...
  for (; AI != AE && PI != PE; ++AI, ++PI) {
    ...
    if (ArgType == ParamType) {
      ...
      if (CastInst::isBitOrNoopPointerCastable(ArgType, ParamType, DL)) {
        ...
        WATEVER_LOG_TRACE("createWrapper: struct param type in bitcast: {}",
                          F->getName());
        WrapperNeeded = false;
      } else {
        WATEVER_LOG_TRACE("createWrapper: arg type mismatch calling: {}",
                          F->getName());
        WATEVER_LOG_TRACE("Arg[{}] Expected: {} Got: {}", Args.size(),
                          llvmToString(*ParamType), llvmToString(*ArgType));
        TypeMismatch = true;
        break;
      }
    }
  ...
  if (WrapperNeeded && !TypeMismatch) {
    ...
    if (RtnType->isVoidTy()) {
      ...
      WATEVER_LOG_TRACE("Creating dummy return: {}", llvmToString(*RtnType));
      ...
      WATEVER_LOG_TRACE("createWrapper: struct return type in bitcast: {}",
                        F->getName());
      WrapperNeeded = false;
    } else {
      WATEVER_LOG_TRACE("crateWrapper: return type mismatch calling: {}",
                        F->getName());
      WATEVER_LOG_TRACE("Expected: {} Got: {}", llvmToString(*ExpectedRtnType),
                        llvmToString(*RtnType));
      TypeMismatch = true;
    }
  }
  ...
  if (TypeMismatch) {
    ...
    WATEVER_LOG_TRACE("createWrapper: no wrapper needed: {}", F->getName());
    Wrapper->eraseFromParent();
    return nullptr;
  }
  WATEVER_LOG_TRACE("createWrapper: {}", F->getName());
  return Wrapper;
}
...
PreservedAnalyses
watever::FixFunctionBitcastsPass::run(Module &M, ModuleAnalysisManager &) {
  WATEVER_LOG_TRACE("Fix Function Bitcasts Pass");
  ...
  for (Function &F : M) {
    ...
    if (F.getName() == "main") {
      ...
      if (shouldFixMainFunction(F.getFunctionType(), MainTy)) {
        WATEVER_LOG_TRACE("Found `main` function with incorrect type: {}",
                          llvmToString(*F.getFunctionType()));
